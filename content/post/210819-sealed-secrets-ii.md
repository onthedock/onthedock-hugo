...
title=  "Creación de Sealed Secrets (SealedSecrets-II)"
date = "2021-08-19T21:12:30+02:00"
+++
...
En esta segunda entrada dedicada a los *SealedSecrets*, me centro en el proceso de creación de un *SealedSecret*.

Las versiones de los elementos con los que he realizado las pruebas son:
...
## Actualizando un valor del *SealedSecret*

En la web oficial de *SealedSecrets* se proporcionan instrucciones sobre cómo actualizar uno de los valores contenidos en un *SealedSecret*: [Update existing secrets](https://github.com/bitnami-labs/sealed-secrets#update-existing-secrets).

Personalmente prefiero usar secretos diferentes para claves/credenciales diferentes, por lo que no es una opción que haya usado por el momento; cuando tengo que actualizar un secreto, lo genero "completo" de nuevo.

En el artículo [Tutorial: Encrypting Kubernetes Secrets with Sealed Secrets](https://www.arthurkoziel.com/encrypting-k8s-secrets-with-sealed-secrets/) se indica, además, cómo añadir valores adicionales o borrar un valor de un `SealedSecret`.

## Resumen

Como comenta Viktor Farcic al final de su [vídeo dedicado a *Sealed Secrets*](https://youtu.be/xd2QoV6GJlc?t=998), resulta alucinante que un producto como *Sealed Secrets* se pueda explicar en unos quince minutos sin dejarse nada en el tintero...

*Sealed Secrets* es conceptualmente sencillo, es fácil crear *SealedSecrets* para poder publicarlos en el repositorio de código, no hace falta modificar las aplicaciones al usar *SealedSecrets*...

*Sealed Secrets* proporciona el **eslabón perdido** que permite implementar *GitOps* convirtiendo el repositorio de código en la **única fuente de verdad** para **TODOS** los recursos en Kubernetes, incluidos los *Secrets*.

Así que ya no tienes excusas: usa *SealedSecrets* y deja de preocuparte por si "alguien" *pushea* credenciales a un repositorio.
